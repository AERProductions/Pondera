...
mob/players
	...
	var // player variables
		...
		// HUD System Manager - manages all UI panels and screen elements
		var/main_hud
		// Toolbelt/Hotbar System - manages equipment quick-access slots
		var/datum/toolbelt/toolbelt
		// Character Creation Data - tracks temporary state during chargen
		list/chargen_data
		// Fishing equipped flag - tracks if fishing tool is equipped
		FRequipped = 0
		// Nutritional status (0-100 scale)
		nutrition = 100
		// Hydration status (0-100 scale)
		hydration = 100
		// Health tracking alias
		health = 100
	...
	Logout()
		// CONSOLIDATED: Save HUD state BEFORE session cleanup
		if(sqlite_ready && src.key)
			SavePlayerHUDState(src)
		
		// Cleanup player session resources
...
*/
	...
	Stat() // these things get called ALOT in order to update stuff
		...
		stat("|<font color = #cd853f>Digging</font>","Acuity: [character.drank] | XP: [character.drankEXP] / [character.drankMAXEXP] | TNL: [(character.drankMAXEXP-character.drankEXP)]")
		...
		stat("|<font color = #5f9ea0>Smelting</font>","Acuity: [character.smerank] | XP: [character.smerankEXP] / [character.smerankMAXEXP] | TNL: [(character.smerankMAXEXP-character.smerankEXP)]")
		stat("|<font color = #e6e8fa>Smithing</font>","Acuity: [character.smirank] | XP: [character.smirankEXP] / [character.smirankMAXEXP] | TNL: [(character.smirankMAXEXP-character.smirankEXP)]")
		...
		stat("|<font color = #f08080>Gardening</font>","Acuity: [character.grank] | XP: [character.grankEXP] / [character.grankMAXEXP] | TNL: [(character.grankMAXEXP-character.grankEXP)]")
		stat("|<font color = #d3d3d3>Mining</font>","Acuity: [character.mrank] | XP: [character.mrankEXP] / [character.mrankMAXEXP] | TNL: [(character.mrankMAXEXP-character.mrankEXP)]")
...
mob/players
	...
	New()
		...
		// Initialize character creation data dict if not set
		if(!chargen_data)
			chargen_data = list()
		// Initialize toolbelt system
		if(!toolbelt)
			InitializeToolbelt(src)

	
	// CONSOLIDATED: Login() moved to HUDManager.dm as canonical implementation
	// HUDManager.dm Login() includes CanPlayersLogin() initialization gate
	// ToggleAdminMode() also integrated into HUDManager.dm Login()

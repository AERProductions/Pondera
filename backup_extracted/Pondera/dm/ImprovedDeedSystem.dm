...
obj/DeedToken_Zone
	...
	proc/SaveToDatabase()
		/**
		 * Phase 5: Persist deed to SQLite database
		 * Called after InitializeTier() or modifications
		 */
	...
	proc/UpdateMaintenanceDatabase()
		/**
		 * Phase 5: Update maintenance status in SQLite
	...
	proc/UpdateFreezeDatabase()
		/**
		 * Phase 5: Update payment freeze status in SQLite
		 * Called when freeze is activated/deactivated
		 */
	...
	proc/InitializeTier(tier)
		...
		// Phase 5: Save to SQLite database
		SaveToDatabase()
	...
	proc/ActivateFreeze(mob/players/M, days)
		...
		// Phase 5: Update freeze status in database

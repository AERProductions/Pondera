...
proc/PruneOldPriceHistory(days_to_keep = 30)
	...
	var/days_in_seconds = days_to_keep * 86400
	...
	if(result)
		world.log << "PRICE_HISTORY_PRUNE: Cleaned up old price history records"
		return 1
...
proc/GetCraftingStatistics(player_id)
	...
	var/spec_result = ExecuteSQLiteQuery(spec_query, list(player_id))
	
	var/discovery_query = "SELECT COUNT(*) as recipes_discovered FROM player_recipe_discovery WHERE player_id=?"
	var/discovery_result = ExecuteSQLiteQuery(discovery_query, list(player_id))
	
	var/achievements_query = "SELECT COUNT(*) as achievements_earned FROM crafting_achievements WHERE player_id=?"
	var/achievements_result = ExecuteSQLiteQuery(achievements_query, list(player_id))
...
proc/LoadRecipeDatabase()
	...
	for(var/row in rows)
		...
		// Store in global RECIPES registry (assumed to exist in game code)
		// RECIPES[recipe_name] = list(type, ingredients, output_item, quality_mod, skill_req)
	
	world.log << "\[Recipes\] Loaded [length(rows)] recipes from database"
	return 1
...
proc/GenerateMarketTrendSnapshot()
	...
	// Get market stats
	var/stats_query = "SELECT COUNT(*) as count, AVG(current_price) as avg_price, AVG(price_volatility) as avg_volatility FROM market_analytics"
	var/stats = ExecuteSQLiteQuery(stats_query)

...
/datum/market_board_manager
	...
	proc/CreateListing(mob/player, item_name, item_type, quantity, price_per_unit, currency_type = "lucre")
		...
		// Phase 2: Also persist to SQLite (async to avoid lag)
		spawn(0) {
			InsertMarketListing(0, player.ckey, item_name, item_type, quantity, price_per_unit, currency_type, "+[listing_duration] seconds")
	...
	proc/PurchaseListing(mob/buyer, listing_id)
		...
		// Phase 2: Update SQLite to mark as purchased (async to avoid lag)
		spawn(0) {
			UpdateMarketListingPurchased(listing_id, 0, buyer.ckey)
			SaveAllMarketListings()
		}
	...
	proc/CancelListing(mob/seller, listing_id)
		...
		// Phase 2: Update SQLite to mark as cancelled (async to avoid lag)
		spawn(0) {

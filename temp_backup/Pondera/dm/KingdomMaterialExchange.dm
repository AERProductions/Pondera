...
// NOTE: /datum/market_price_tracker removed in Phase 0
// Prices now stored in SQLite market_prices table (see Phase 1)
// Use GetMarketPrice(commodity_name) proc instead of direct variable access
...
var
	...
	// market_prices datum removed - see DynamicMarketPricingSystem.dm for GetMarketPrice() API

/proc/InitializeKingdomMaterialExchange()
	/**
	 * InitializeKingdomMaterialExchange()
	 * Sets up kingdom trading system on world boot
	 * Market prices are now initialized via InitializeMarketPrices() in SQLitePersistenceLayer.dm (Phase 1)
	 */
...
/proc/UpdateMarketSupplyFromTrade(datum/kingdom_trade_offer/offer)
	 ...
	 * DEPRECATED in Phase 0: Market prices now managed by SQLite (see DynamicMarketPricingSystem.dm)
	 */
	// Placeholder - no-op until Phase 1 SQLite integration
	...
	// Supply tracking updated (will be handled by SQLite in Phase 1)
	// TODO: Update to use GetMarketPrice() and UpdateMarketPrice() procs
...
/proc/AdjustMarketPricesFromSupply()
	 ...
	 * DEPRECATED in Phase 0: This proc will be replaced by SQLite-backed implementation
	 * Currently a placeholder - no-op until Phase 1
	 * More supply = lower prices, less supply = higher prices
	 */
	// Prices now managed by SQLite market_prices table
	// See DynamicMarketPricingSystem.dm for price update logic

/proc/GetMarketPrices()
	/**
	 * GetMarketPrices()
	 * DEPRECATED in Phase 0 - returns null
	 * Use GetCommodityPrice(commodity_name) instead (defined in DynamicMarketPricingSystem.dm)
	 */
	return null
...
/proc/GetKingdomNetWorth(kingdom_name)
	...
	// Get prices using unified API
	total += tm.stone_treasury * GetCommodityPrice("stone")
	total += tm.metal_treasury * GetCommodityPrice("metal")
	total += tm.timber_treasury * GetCommodityPrice("timber")
...
/proc/GetKingdomTreasuryStatus(kingdom_name)
	...
	// Get current prices using unified API
	var/stone_price = GetCommodityPrice("stone")
	var/metal_price = GetCommodityPrice("metal")
	var/timber_price = GetCommodityPrice("timber")
	...
	var/report = list(
		...
		"market_prices" = list(
			"stone" = round(stone_price, 0.01),
			"metal" = round(metal_price, 0.01),
			"timber" = round(timber_price, 0.01)
		)
	)

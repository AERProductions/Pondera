...
/obj/items/equipment/proc/IsBroken()
	/// Returns TRUE if item durability is at 0
	return (current_durability <= 0)

/obj/items/equipment/proc/IsFragile()
	/// Returns TRUE if item durability is below 25%
	if(max_durability > 0)
		return (current_durability < (max_durability * 0.25))
	return FALSE

/obj/items/equipment/proc/GetDurabilityPercent()
	/// Returns durability as percentage (0-100)
	if(max_durability <= 0) return 100
	return round((current_durability / max_durability) * 100)

/obj/items/equipment/proc/AttemptUse(amount = 1)
	/// Reduces durability by amount; returns success
